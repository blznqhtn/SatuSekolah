...
@section('content')
    <div class="container mx-auto px-4 sm:px-6 lg:px-8 py-6 mt-6 sm:mt-10">
        <div class="mb-8 text-center sm:text-left">
            ...
            <p class="text-gray-500 dark:text-gray-400 mt-1 flex items-center justify-center sm:justify-start gap-2">
                <i class="fa-regular fa-pen-to-square text-primary-500"></i>
                Perbarui data member yang sudah terdaftar
            </p>
        </div>
        ...
        <div class="max-w-5xl mx-auto">
            <form action="{{ route('admin.siswa.update', $siswa->id) }}" method="POST" enctype="multipart/form-data"
                ...
                <input type="hidden" id="original_no_induk" name="original_no_induk" value="{{ $siswa->nomor_induk }}">
                ...
                <div
                            ...
                            <div class="space-y-6">
                                <div>
                                    ...
                                    <input type="text" id="no_induk" name="no_induk" placeholder="Masukkan No Induk"
                                        class="w-full px-4 py-2.5 rounded-xl border border-gray-200 dark:border-gray-600 bg-gray-100 dark:bg-gray-700 text-gray-500 dark:text-gray-400 cursor-not-allowed focus:ring-2 focus:ring-primary-500 focus:border-transparent transition-all duration-200 @error('no_induk') border-red-500 @enderror"
                                        required readonly value="{{ old('no_induk', $siswa->nomor_induk) }}" />
                                    @error('no_induk')
                                ...
                                <div>
                                    ...
                                    <select id="class" name="class"
                                        class="w-full px-4 py-2.5 rounded-xl border border-gray-200 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-primary-500 focus:border-transparent transition-all duration-200 cursor-pointer @error('class') border-red-500 @enderror"
                                        required>
                                        <option value="" disabled>Pilih kelas</option>
                                        @foreach ($dataKelas as $kelas)
                                            <option value="{{ $kelas->id }}"
                                                {{ old('class', $siswa->id_kelas) == $kelas->id ? 'selected' : '' }}>
                                                {{ $kelas->name }}
                                            </option>
                                        @endforeach
                                    </select>
                                ...
                                <div>
                                    ...
                                    <textarea name="address" id="address" rows="4"
                                        class="w-full px-4 py-2.5 rounded-xl border border-gray-200 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-primary-500 focus:border-transparent transition-all duration-200 @error('address') border-red-500 @enderror"
                                        placeholder="Masukkan alamat rumah">{{ old('address', $siswa->alamat) }}</textarea>
                                    @error('address')
                                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                                    @enderror
                                </div>
                                ...
                                <div class="relative">
                                    ...
                                    hover:border-primary-500 dark:hover:border-primary-500 group inline-block w-full">
                                        ...
                                        <div id="upload-placeholder"
                                            class="flex flex-col items-center justify-center cursor-pointer {{ $siswa->foto_profile ? 'hidden' : '' }}">
                                            <div
                                            ...
                                            <button type="button" onclick="openPhotoBrowser()"
                                                class="px-4 py-1.5 bg-linear-to-r from-primary-600 to-primary-700 hover:from-primary-700 hover:to-primary-800 text-white text-sm font-medium rounded-lg shadow-md transition-all duration-300 hover:scale-105 cursor-pointer">
                                                Pilih Foto
                                            </button>
                                        </div>
                                        ...
                                        <div id="photo-preview-container"
                                            class="relative w-36 mx-auto {{ $siswa->foto_profile ? '' : 'hidden' }}">
                                            <img id="photo-preview" src="{{ $siswa->foto_profile }}" alt="Preview"
                                    ...
                                    <span id="current_photo_name"
                                        class="text-sm text-gray-500 dark:text-gray-400 mt-2 inline-block">
                                        {{ $siswa->foto_profile ? Str::after($siswa->foto_profile, '/profile/') : 'No photo selected' }}
                                    </span>
                                    @error('photo')
                                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                                    @enderror
                    ...
                    <div
                        ...
                        <button type="submit"
                            class="inline-flex justify-center items-center px-6 py-2.5 bg-linear-to-r cursor-pointer from-primary-600 to-primary-700 hover:from-primary-700 hover:to-primary-800 text-white font-semibold rounded-xl shadow-md transition-all duration-300 hover:scale-105 active:scale-95">
    ...
    <!-- Modal Browser Foto Modern -->
    <div id="photoBrowserModal" class="fixed inset-0 z-60 hidden overflow-y-auto backdrop-blur-md">
...
@push('scripts')
    <script>
        // ==================== LOGIKA UPLOAD FOTO & PREVIEW (SAMA PERSIS DENGAN ASLINYA) ====================
        ...
        // ==================== PHOTO BROWSER (SAMA PERSIS DENGAN ASLINYA) ====================
