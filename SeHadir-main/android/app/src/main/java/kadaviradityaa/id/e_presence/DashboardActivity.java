...
public class DashboardActivity extends AppCompatActivity {
    ...
    private void loadUserData() {
        Module.init(this);
        // Pastikan URL sesuai dengan routes/api.go (users group)
        String url = Module.urlKoneksi + "api/users/getMyAccount/" + sharedPreferences.getString("uid", "");
        String token = sharedPreferences.getString("token", "");

        Module.getObjectWithToken(this, url, token, response -> {
            try {
                if (response.getString("status").equals("success")) {
                    txtNama.setText(response.optString("name", "Unknown"));
                    txtProfileName.setText(response.optString("name", "Unknown"));
                    txtClass.setText(response.optString("kelas", "-"));
                    txtNIS.setText(response.optString("no_induk", "-"));

                    // Menggunakan String.valueOf karena backend mengirim int64
                    txtTotalHadir.setText(String.valueOf(response.optInt("total_hadir_bulan_ini", 0)));
                    txtTotalAlpa.setText(String.valueOf(response.optInt("total_alpa_bulan_ini", 0)));
                    txtIzinSakit.setText(String.valueOf(response.optInt("total_izin_bulan_ini", 0)));

                    String profileUrl = response.optString("profile", "");
                    ...
                    Glide.with(this)
                            .load(response.optBoolean("absen_hari_ini_status", false) ? R.drawable.ix_success_filled : R.drawable.ic_sharp_do_disturb_on)
                            .placeholder(R.drawable.ic_sharp_do_disturb_on)
                            .error(R.drawable.ic_sharp_do_disturb_on)
                            .into(statusAbsensiBadge);

                    btnLinkCard.setVisibility(response.optBoolean("rfid_status", false) ? View.GONE : View.VISIBLE);
                    btnLinkCard.setEnabled(!response.optBoolean("rfid_status", false) && isNfcSupported(this));

                    // Logika tombol "Buat Surat" Realtime (Hanya muncul jika belum absen DAN belum lewat jam batas Alpha)
                    boolean isAlreadyAbsent = response.optBoolean("absen_hari_ini_status", false);
                    boolean canCreateLeave = response.optBoolean("can_create_leave", false);

                    isAbsensi = isAlreadyAbsent;
                    btnBuatSurat.setVisibility((isAlreadyAbsent || !canCreateLeave) ? View.GONE : View.VISIBLE);
                    btnBuatSurat.setEnabled(!isAlreadyAbsent && canCreateLeave);
