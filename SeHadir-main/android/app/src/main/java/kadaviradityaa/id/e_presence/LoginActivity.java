            ...
            if (tag != null) {
                ...
                if (isMifareCard && signStatus) {
                    ...
                    if (!formattedUid.isEmpty()) {
                        ...
                        try {
                            postDatas.put("type", "rfid");
                            postDatas.put("rfid_id", formattedUid);
                        } catch (JSONException e) {
                            throw new RuntimeException(e);
                        }

                        Module.postObject(this, Module.urlKoneksi + "api/auth/login", postDatas,
                                response -> {
                                    String token;
                                    try {
            ...
            if(signStatus){
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.VANILLA_ICE_CREAM) {
                    if(!Objects.requireNonNull(txtUsername.getText()).isEmpty() && !Objects.requireNonNull(txtPass.getText()).isEmpty()){
                        ...
                        Module.postObject(this, Module.urlKoneksi + "api/auth/login", postData,
                            response -> {
                                String token;
                                try {
                                    if(response.getString("status").equals("success")){
                ...
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.VANILLA_ICE_CREAM) {
                    if(!Objects.requireNonNull(txtUsername.getText()).isEmpty() && !Objects.requireNonNull(txtMail.getText()).isEmpty() && !Objects.requireNonNull(txtPass.getText()).isEmpty() && !Objects.requireNonNull(txtNIS.getText()).isEmpty()){
                        ...
                        Module.postObject(this, Module.urlKoneksi + "api/auth/daftar-akun", postData,
                                response -> {
                                    try {
                                        if(response.getString("status").equals("success")){
                                            ...
                                            btnLanjut.setOnClickListener(view -> {
                                                ...
                                                Module.postObject(this, Module.urlKoneksi + "api/auth/register-account", sendObj,
                                                    response1 -> {
                                                        try {
