...
('p-014', 'MANAGE_PKG'),
('p-015', 'MANAGE_PKL'),
('p-016', 'MANAGE_CANTEEN'),
('p-017', 'MANAGE_GATEPASS'),
('p-018', 'MANAGE_COMMUNICATION'),
('p-019', 'MANAGE_MENTOR');
...
CREATE TABLE student_invoices (
      ...
      -- Kode pembayaran dinamis (berlaku s.d. due_date)
      dynamic_qr_code VARCHAR(100) UNIQUE,
      rfid_payment_code VARCHAR(20) UNIQUE,
      transfer_target_account VARCHAR(20) UNIQUE,
      -- Midtrans order ID untuk pembayaran tunai
...
CREATE TABLE job_vacancies (
    ...
    target_role ENUM('STUDENT', 'TEACHER', 'STAFF', 'ANY') DEFAULT 'STUDENT',
...
-- ==========================================
-- 26. CBA (COMPUTER BASED ASSESSMENT)
-- ==========================================

-- MANAGE_CBA permission already seeded at p-005 above

-- Exam Folders/Categories (e.g. UTS, UKK, Ulangan Harian)
-- Token is auto-generated every 5 minutes by the server and shared to students.
CREATE TABLE IF NOT EXISTS cba_folders (
    id CHAR(36) PRIMARY KEY,
    tenant_id CHAR(36) NOT NULL,
    name VARCHAR(255) NOT NULL,
    description TEXT,
    current_token VARCHAR(12) NOT NULL,   -- 6-char alphanumeric, refreshed every 5 min
...
-- Individual Exams inside a folder
CREATE TABLE IF NOT EXISTS cba_exams (
    id CHAR(36) PRIMARY KEY,
    folder_id CHAR(36) NOT NULL,
    tenant_id CHAR(36) NOT NULL,
    title VARCHAR(255) NOT NULL,
    terms_and_conditions TEXT,            -- T&C shown in waiting room
    start_time TIMESTAMP NOT NULL,        -- Server-authoritative start time
    end_time TIMESTAMP NOT NULL,          -- Server-authoritative end time (all students end simultaneously)
    waiting_room_open_minutes INT DEFAULT 5, -- How many minutes before start_time students can enter
    show_results BOOLEAN DEFAULT FALSE,   -- TRUE = show score immediately; FALSE = hidden
    created_by CHAR(36) NOT NULL,
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    deleted_at TIMESTAMP,
    FOREIGN KEY (folder_id) REFERENCES cba_folders(id) ON DELETE CASCADE,
    FOREIGN KEY (tenant_id) REFERENCES tenants(id) ON DELETE CASCADE,
    FOREIGN KEY (created_by) REFERENCES users(id) ON DELETE CASCADE
);

-- Questions belonging to an exam
CREATE TABLE IF NOT EXISTS cba_questions (
    id CHAR(36) PRIMARY KEY,
    exam_id CHAR(36) NOT NULL,
    order_index INT NOT NULL DEFAULT 0,   -- For ordered display in navigation panel
    question_type VARCHAR(50) NOT NULL,   -- MULTIPLE_CHOICE, ESSAY, MATCHING
    question_text TEXT NOT NULL,
    media_url VARCHAR(500),
    media_type VARCHAR(50) DEFAULT 'NONE',
    score_weight DECIMAL(5,2) DEFAULT 1.0,
    ai_reference_answer TEXT,             -- For AI auto-grading of ESSAY type
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    FOREIGN KEY (exam_id) REFERENCES cba_exams(id) ON DELETE CASCADE
);

-- Answer options for multiple choice questions
CREATE TABLE IF NOT EXISTS cba_options (
    id CHAR(36) PRIMARY KEY,
    question_id CHAR(36) NOT NULL,
    option_text TEXT,
    media_url VARCHAR(500),
    ...
    is_correct BOOLEAN DEFAULT FALSE,
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    FOREIGN KEY (question_id) REFERENCES cba_questions(id) ON DELETE CASCADE
);

-- Exam session per student (one session per student per exam)
CREATE TABLE IF NOT EXISTS cba_sessions (
    id CHAR(36) PRIMARY KEY,
    exam_id CHAR(36) NOT NULL,
    student_id CHAR(36) NOT NULL,
    waiting_room_joined_at TIMESTAMP,     -- When student entered the waiting room
    started_at TIMESTAMP,                 -- When student officially started
    submitted_at TIMESTAMP,               -- When session was finalized (manual or auto)
    is_auto_submitted BOOLEAN DEFAULT FALSE,
    final_score DECIMAL(5,2),             -- Calculated and stored on submission
    status VARCHAR(20) DEFAULT 'WAITING', -- WAITING, ONGOING, SUBMITTED
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
...
-- Student answers (saved progressively during exam)
CREATE TABLE IF NOT EXISTS cba_answers (
    id CHAR(36) PRIMARY KEY,
    session_id CHAR(36) NOT NULL,
    question_id CHAR(36) NOT NULL,
    option_id CHAR(36),                   -- NULL for ESSAY answers
    essay_text TEXT,                      -- For ESSAY type
    is_doubt BOOLEAN DEFAULT FALSE,       -- "Ragu-ragu" flag
    answered_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    UNIQUE(session_id, question_id),      -- One answer per question per session (upsert)
    FOREIGN KEY (session_id) REFERENCES cba_sessions(id) ON DELETE CASCADE,
	FOREIGN KEY (question_id) REFERENCES cba_questions(id) ON DELETE CASCADE,
	FOREIGN KEY (option_id) REFERENCES cba_options(id) ON DELETE SET NULL
);

-- ==========================================
-- 23. COMMUNICATION (WebSocket Chat)
-- ==========================================
...
CREATE TABLE IF NOT EXISTS chat_messages (
    ...
    ciphertext TEXT NOT NULL,          -- AES-256-GCM encrypted message content
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
...
CREATE TABLE IF NOT EXISTS inventory_items (
    ...
    description TEXT,
    stock INT DEFAULT 0,
    condition VARCHAR(50) DEFAULT 'GOOD', -- e.g. GOOD, NEEDS_REPAIR, BROKEN
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
);
...
-- ==========================================
-- 23. VIEWS (PERFORMANCE OPTIMIZATION)
-- ==========================================
